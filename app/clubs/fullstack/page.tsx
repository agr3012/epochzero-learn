// app/clubs/fullstack/page.tsx - same pattern as REMA, different color + activities
import Image from 'next/image';
import Link from 'next/link';
import { Shield, Users, Trophy, Code2, ChevronRight, Calendar, MapPin, ExternalLink, Database, Cloud, Layers, Cpu, Lock, Paintbrush } from 'lucide-react';
import { createClient } from '@/lib/supabase/server';
import { FSDRegistrationForm } from './FSDRegistrationForm';

export const revalidate = 3600;
export const metadata = { title: 'Full Stack Development Club — Build. Deploy. Scale.' };

const COLOR = '#1B5FA8';
const FSD_LOGO = 'https://nqyruorkiqaomqzgixgo.supabase.co/storage/v1/object/public/club/FSD_Club_Logo.png';

// Re-use all sub-components from REMA page — they're parameterised by `color`
// (In production, extract them to a shared file: components/clubs/shared.tsx)
// For now they are duplicated below for build independence.
    ...
    <div style={{ minHeight: '100vh' }}>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <nav className="flex items-center gap-2 text-sm mb-6" style={{ color: 'hsl(var(--foreground-muted))' }}>
                ...
                <span style={{ color: COLOR }}>Full Stack Development Club</span>
              </nav>
              ...
              <h1 className="font-display font-bold leading-tight mb-3"
                ...
                Full Stack<br /><span style={{ color: COLOR }}>Dev Club</span>
              </h1>
              <p className="font-sans text-lg tracking-wide mb-5" style={{ color: COLOR }}>Build. Deploy. Scale.</p>
              <p className="font-serif text-lg leading-relaxed max-w-lg mb-8" style={{ color: 'hsl(var(--foreground-muted))' }}>{club.description}</p>
              <div className="flex flex-wrap gap-3 mb-10">
                <a href="#register" className="btn-primary"><Code2 className="w-4 h-4" /> Join the Club</a>
                <a href="#events"   className="btn-ghost"><Trophy className="w-4 h-4" /> Our Hackathons</a>
              </div>
              <div className="flex gap-8">
                {[{ v: members.length, l: 'Members' }, { v: events.length || '—', l: 'Events' }, { v: club.founded_year, l: 'Founded' }].map(({ v, l }) => (
            ...
            <div className="flex items-center justify-center lg:justify-end">
              <Image src={FSD_LOGO} alt="FSD Club Logo" width={280} height={280} className="drop-shadow-2xl" priority />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="container py-16">
        ...
        {current.length === 0
          ...
          : <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">{current.map(m => <MemberCard key={m.id} m={m} color={COLOR} active />)}</div>}
      </section>
      ...
      {alumni.length > 0 && (<>
        ...
        <section className="container py-16">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">{alumni.map(m => <MemberCard key={m.id} m={m} color={COLOR} />)}</div>
        </section>
      </>)}
      ...
      <section id="events" className="container py-16 scroll-mt-20">
        ...
        <h2 className="font-display text-2xl font-semibold mb-8" style={{ color: 'hsl(var(--foreground))' }}>Hackathons & Events</h2>
        {events.length === 0
          ? <div className="card p-12 text-center" style={{ border: '1px dashed hsl(var(--border))' }}>
              ...
              <p className="text-sm mb-1" style={{ color: 'hsl(var(--foreground-muted))' }}>Our first hackathon is being planned. Watch this space.</p>
              <p className="text-xs" style={{ color: 'hsl(var(--foreground-subtle))' }}>Code-to-Cloud Sprint — coming soon</p>
            </div>
          : <div className="space-y-4">{events.map((ev, i) => <EventCard key={ev.id} ev={ev} color={COLOR} index={events.length - i} />)}</div>}
      </section>
      ...
      {projects.length > 0 && (<>
        ...
        <section className="container py-16">
          ...
          <div className="grid lg:grid-cols-2 gap-4">{projects.map(p => <ProjectCard key={p.id} p={p} color={COLOR} />)}</div>
        </section>
      </>)}
      ...
      <section style={{ background: 'hsl(var(--surface))' }}>
        <div className="container py-16">
          ...
          <MentorCard club={club} color={COLOR}
            roles={['Researcher · ML & Cybersecurity', 'Club Founder & Mentor']}
            bio="Founder and mentor of the Full Stack Development Club at SITAICS, RRU. Active researcher in ML and cybersecurity with extensive experience teaching and building full-stack applications. Brings industry-grade development practices directly into the classroom."
            highlights={[
      ...
      <section id="register" className="container py-16 scroll-mt-20">
        ...
        <h2 className="font-display text-2xl font-semibold mb-3" style={{ color: 'hsl(var(--foreground))' }}>Join the Full Stack Development Club</h2>
        <p className="font-serif text-base leading-relaxed max-w-2xl mb-10" style={{ color: 'hsl(var(--foreground-muted))' }}>
          Open to all RRU students. Use your official RRU email (<span className="font-mono text-sm" style={{ color: COLOR }}>@student.rru.ac.in</span> or <span className="font-mono text-sm" style={{ color: COLOR }}>@rru.ac.in</span>).
        </p>
        <FSDRegistrationForm clubId={club.id} clubName={club.name} />
        <RegistrationInfo color={COLOR}
          benefits={['Access to exclusive hackathon challenges','Hands-on full-stack project sprints','Mentorship from faculty and industry professionals','Architecture teardown and briefing sessions','Certificate of membership and participation','Portfolio-ready real-world projects']}
        />
      </section>
    </div>
  );
...
// ── Sub-components (same as REMA — in production extract to shared file) ──
  ...
  return (
    <div className="card p-6">
      <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-4 font-display text-lg font-bold text-white"
        style={{ background: active ? color : 'hsl(var(--border-strong))' }}>{initials}</div>
      ...
      {active && (
        <div className="mt-3 inline-flex items-center gap-1.5 text-xs px-2.5 py-0.5 rounded-full"
          ...
          <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: color }} />Active
        </div>
      )}
    </div>
  );
    ...
    <div className="card p-6 lg:p-8">
      <div className="grid lg:grid-cols-[1fr_200px] gap-6">
        <div>
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            ...
            <span className={`font-sans text-xs font-medium px-2.5 py-0.5 rounded-full ${ev.status === 'upcoming' ? 'bg-green-500/10 text-green-500 border border-green-500/30' : ''}`}
              style={ev.status !== 'upcoming' ? { background: 'hsl(var(--muted))', color: 'hsl(var(--foreground-muted))', border: '1px solid hsl(var(--border))' } : {}}>{ev.status}</span>
          </div>
        ...
        <div className="card p-4 flex flex-col gap-3 text-xs self-start" style={{ borderLeft: `3px solid ${color}` }}>
          {ev.event_date && <div className="flex items-center gap-2" style={{ color: 'hsl(var(--foreground-muted))' }}><Calendar className="w-3.5 h-3.5 shrink-0" style={{ color }} />{new Date(ev.event_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}</div>}
          {ev.venue      && <div className="flex items-start gap-2" style={{ color: 'hsl(var(--foreground-muted))' }}><MapPin className="w-3.5 h-3.5 shrink-0 mt-0.5" style={{ color }} /><span className="leading-snug">{ev.venue}</span></div>}
          {ev.report_url && <a href={ev.report_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 hover:underline" style={{ color }}><ExternalLink className="w-3 h-3 shrink-0" />View report</a>}
          <div className="font-display text-3xl font-bold pt-3 mt-1" style={{ borderTop: '1px solid hsl(var(--border))', color: 'hsl(var(--border-strong))' }}>#{String(index).padStart(2,'0')}</div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="card p-6 lg:p-8">
      <div className="flex items-start justify-between gap-4 mb-4">
        <span className="font-sans text-xs font-medium px-2.5 py-0.5 rounded-full"
          style={p.status === 'active' ? { background: `${color}18`, color, border: `1px solid ${color}40` } : { background: 'hsl(var(--muted))', color: 'hsl(var(--foreground-muted))', border: '1px solid hsl(var(--border))' }}>
          {p.status}
        </span>
        {p.github_url && <a href={p.github_url} target="_blank" rel="noopener noreferrer" className="hover:text-[hsl(var(--primary))] transition-colors" style={{ color: 'hsl(var(--foreground-muted))' }}><ExternalLink className="w-4 h-4" /></a>}
      </div>
      ...
      {Array.isArray(p.tech_stack) && p.tech_stack.length > 0 && (
        <div className="flex flex-wrap gap-2">{p.tech_stack.map((t: string) => <span key={t} className="font-mono text-xs px-2 py-0.5 rounded" style={{ background: 'hsl(var(--muted))', color: 'hsl(var(--foreground-muted))', border: '1px solid hsl(var(--border))' }}>{t}</span>)}</div>
      )}
    </div>
  );
}
  ...
  return (
    <div className="card rounded-xl overflow-hidden grid lg:grid-cols-[260px_1fr]">
      <div className="p-10 flex flex-col items-center justify-center text-center" style={{ background: `${color}12`, borderRight: '1px solid hsl(var(--border))' }}>
        ...
        <div className="flex flex-col gap-2 w-full">{roles.map(r => <span key={r} className="text-xs px-3 py-1.5 rounded-lg" style={{ background: 'hsl(var(--card))', color: 'hsl(var(--foreground-muted))', border: '1px solid hsl(var(--border))' }}>{r}</span>)}</div>
      </div>
      <div className="p-10">
        ...
        <div className="grid sm:grid-cols-2 gap-4 mb-8">{highlights.map(({ label, value }) => <div key={label} className="pl-4" style={{ borderLeft: `2px solid ${color}40` }}><div className="font-sans text-xs font-semibold uppercase tracking-wide mb-1" style={{ color: 'hsl(var(--foreground-muted))' }}>{label}</div><div className="font-serif text-sm" style={{ color: 'hsl(var(--foreground))' }}>{value}</div></div>)}</div>
...
function RegistrationInfo({ color, benefits }: { color: string; benefits: string[] }) {
  return (
    <div className="grid md:grid-cols-3 gap-5 mt-14 pt-12" style={{ borderTop: '1px solid hsl(var(--border))' }}>
      <div className="card p-6">
        <h4 className="font-sans font-semibold text-sm mb-4" style={{ color }}>What happens next</h4>
        <ol className="space-y-3">{['Submit the form with your RRU email address.','The club mentor reviews within 1-2 working days.','You receive an approval email on your RRU address.','Approved members are added to the club group.'].map((text, n) => <li key={n} className="flex gap-3"><span className="font-display font-bold text-sm shrink-0" style={{ color: `${color}60` }}>0{n+1}</span><span className="font-serif text-sm" style={{ color: 'hsl(var(--foreground-muted))' }}>{text}</span></li>)}</ol>
      </div>
      <div className="card p-6">
        <h4 className="font-sans font-semibold text-sm mb-4" style={{ color }}>As a member you get</h4>
        <ul className="space-y-2">{benefits.map(item => <li key={item} className="flex gap-2 font-serif text-sm" style={{ color: 'hsl(var(--foreground-muted))' }}><span style={{ color }} className="shrink-0">·</span>{item}</li>)}</ul>
      </div>
      <div className="card p-6">
        <h4 className="font-sans font-semibold text-sm mb-4" style={{ color }}>Questions?</h4>
        <p className="font-serif text-sm leading-relaxed mb-4" style={{ color: 'hsl(var(--foreground-muted))' }}>Contact the club mentor at SITAICS, RRU Gandhinagar, or reach out through EpochZero.</p>
        <Link href="/about" className="inline-flex items-center gap-1.5 text-sm font-medium hover:gap-2 transition-all" style={{ color }}>Contact <ChevronRight className="w-3.5 h-3.5" /></Link>
      </div>
    </div>
  );
}
