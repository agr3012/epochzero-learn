// app/clubs/rema/RegistrationForm.tsx
'use client';
...
const INTERESTS = [
  { value: 'ctf',     label: 'Participating in CTF competitions'                              },
...
export function ClubRegistrationForm({ clubId }: { clubId: string; clubName: string }) {
  ...
  if (status === 'success') return (
    <div className="max-w-2xl card p-12 text-center" style={{ borderTop: `3px solid ${COLOR}` }}>
      ...
      <p className="font-serif text-sm leading-relaxed" style={{ color: 'hsl(var(--foreground-muted))' }}>
        Your application to the REMA Club has been received. The mentor will notify you at your RRU email.
      </p>
    </div>
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-10 max-w-4xl" noValidate>
      ...
      <div className={twoCol}>
        <Field label="Have you participated in any previous REMA CTFs?" error={errors.ctf_participation} required color={COLOR}>
      ...
      <Field label="Why do you want to join and what do you hope to achieve?" error={errors.statement} required color={COLOR}>
        <textarea value={form.statement} onChange={e => set('statement', e.target.value)}
          rows={5} placeholder="Describe your motivation, goals, and what you can contribute..."
      ...
      <div className="card p-5 mb-4">
        <p className="font-serif text-sm leading-relaxed" style={{ color: 'hsl(var(--foreground-muted))' }}>
          By joining the REMA Club, I agree to adhere to the ethical guidelines set forth by SITAICS and RRU. I understand that the tools and techniques discussed are for educational and defensive purposes only.
        </p>
      </div>
